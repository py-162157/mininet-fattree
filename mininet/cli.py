...
class CLI( Cmd ):
    ...
    def do_iperfbisection(self, line):
        "added by pengyang, to implement bisection throughput test."
        args=line.split()
        if len(args) == 1:
            udpBw = args[ 0 ]
            self.mn.iperfbisection(udpBw)
        elif len(args)== 2:
            udpBw = args[ 0 ]
            period = args[ 1 ]
            err = False
            self.mn.iperfbisection(udpBw,int(period))
        else:
            error('invalid number of args: iperfbisection udpBw \n' +'udpBw examples: 1M\n')
