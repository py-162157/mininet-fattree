...
class Mininet( object ):
    ...
    def iperfbisection( self, load, period=5):
        """added by pengyang, bisection iperf UDP test between nodes"""
        base_port = 5001
        #output(load)
        server_list = []
        ...
        i = 0
        k = 0
        while k*k*k/4 != len(host_list):
            k+=1

        for host in host_list:
            switch_id_string = host.name.split("s")
            print("switch_id_string =", switch_id_string[1])
            switch_id = int(switch_id_string[1])
        ...
        for i in range(len(senders)):
            podpair[senders[i]] = receivers[i]

        for client in podpair:
            server = podpair[client]
            t = threading.Thread(target=self.iperf_single, args=([client, server], load, period, base_port))
            t.start()
    ...
    def iperf_single( self,hosts=None, udpBw=5, period=5, port=5001):
        ...
        iperfArgs = 'iperf -u'    
        print("****start server******")
        server.cmd( iperfArgs + '-s' + ' >> /home/pengyang/log/' + 'server-'+filenameserver + '&')
        print("****start client******")
        client.cmd(iperfArgs + '-t ' + str(period) + ' -c ' + server.IP() + ' -b '+str(udpBw)+'M'+ ' >> /home/pengyang/log/'+ 'client-' + filename +'&')
